...
async def _(bot: Bot, event: MessageEvent, state: T_State = State()):
    ...
    try:
        time_tup = time.strptime(event.normalize_dict()['source']['time'], "%Y-%m-%dT%H:%M:%S+00:00")
        time_stamp = time.mktime(time_tup)
        cd = time_stamp - cd_dir[qid]
    except KeyError:
        cd = cdTime + 1

    # 先判断r18flag和私聊是不是都是True进行赋值
    r18 = True if (isinstance(event, FriendMessage)
    ...
    if (
        ...
        for pic in data:
            ...
            if pic[2]:
                message = f"{random.choice(setu_sendmessage)}{flagLog}" + \
                          MessageSegment.plain(pic[1]) + MessageSegment.image(base64=pic[0])
        ...
        # 发送失败
            ...
            cd_dir.pop(qid)
            await setu.finish(
                message=MessageSegment.plain(f"消息被风控了捏，图发不出来，请尽量减少发送的图片数量"),
                at_sender=True,
            )
