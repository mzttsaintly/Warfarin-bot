...
def circle_corner(img, radii):  # 把原图片变成圆角，
    ...
    # 创建二维码
    # new_qrcode = Image.new("RGB", (300, 300), (255, 255, 255))
    # 创建二维码的Draw对象
    # draw = ImageDraw.Draw(new_qrcode)


async def build_get_share_info(video_info: VideoInfo):
    """
    构建获取分享链接的图片
    :param video_info: 视频id
    :return: 图片
    """
    if not video_info:
        return None
    ...
    if img_width - 50 < font.getsize(title)[0]:
        ...
        for i in range(len(title)):
            img_sum += font.getsize(title[i])[0]
            if img_sum > img_width - 50:
                title = title[: i - 1] + "···"
                break
    ...
    if font.getsize(desc)[0] > (img_width - 50) * 2:
        ...
        for i in range(len(desc)):
            img_sum += font.getsize(desc[i])[0]
            if img_sum > (img_width - 50) * 2:
                # 将简介字符超出的部分删除
                desc = desc[: i - 5] + "···"
                break
            if img_sum > (img_width - 50):
...
async def build_video_poster(video_info: VideoInfo):
    ...
    # 视频信息
    tmp = f"{format_number(video_info.stat.view)}播放·" \
          f" {format_number(video_info.stat.like)}点赞 · " \
          f"{format_number(video_info.stat.danmaku)}弹幕"
    font = ImageFont.truetype(
